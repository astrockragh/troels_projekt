...
# Two 1D arrays:
Nposi_aarhus = data_posi.loc["2020-03-01":
                             "2020-09-22"][area_aarhus].values.sum(axis=1)
Ntest_aarhus = data_test.loc["2020-03-01":
                             "2020-09-22"][area_aarhus].values.sum(axis=1)
...
Ntest_aarhus, Ntest_cph = Ntest_aarhus[:-1], Ntest_cph[:-1]
...
enPos_aarhus = nAveDailyTests_aarhus * \
    np.sqrt(fPos_aarhus*(1.0-fPos_aarhus)/Ntest_aarhus + SystError**2)
...
ax_Ntests.set(xlabel="Day (1st of March is day 0)",
              ylabel="Tests / day", title="")
ax_Ntests.errorbar(day[Plot_StartDay:Plot_EndDay], Ntest_aarhus[Plot_StartDay:Plot_EndDay],
                   yerr=eNtest_aarhus[Plot_StartDay:Plot_EndDay], fmt='.', linewidth=2, label='N tests in Aarhus', color='red')
...
if (doFitSIR):
    ...
    minuit_SIR = Minuit(ChiSquareCalcSIR, pedantic=False, print_level=0,
                        dayL=dayLockdown_aarhus, nI0=2.0, beta0=0.45, beta1=0.04, gamma=0.14, fix_gamma=True)
    minuit_SIR.migrad()
...
if (doFitSEIR):
    ...
    minuit_SEIR = Minuit(ChiSquareCalcSEIR, pedantic=False, print_level=0, dayL=dayLockdown_aarhus-4, fix_dayL=True,
